...
weather_plot <- ggplot(divy_2017, aes(events, log_tripduration, fill = events)) +
  ...
  xlab("Weather event") +
  ylab("Log(trip duration)") +
  ggtitle("Log(trip duration) vs. weather event")

plot_grid(temp_plot, weather_plot, labels = "AUTO")


```  
Based on the visualisations, for


#### a: Trip distances vs. temperature (ºF)
...
Below is a boxplot visualisation of bike trip durations vs gender:
...
ggplot(divy_2017, aes(gender, log_tripduration, fill = gender)) +
  ...
  xlab("Gender") +
  ylab("Log(trip duration)") +
  ggtitle("Log(trip duration) vs. gender")
...
### Part 4: A predictive model for Divvy bike trip durations?

Based on the results from the analyses above, this is a potential multivariate model that we could use to predict Divvy bike trip durations.

```{r}

full_model <- lm(log_tripduration ~ temperature + temperature:events + gender + hour*day + hour*month + day*month)
